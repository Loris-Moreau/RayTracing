...
HittableList random_scene()
{
    ...
    std::shared_ptr<CheckerTexture> checker = std::make_shared<CheckerTexture>(std::make_shared<SolidColor>(0.2, 0.3, 0.1),
        ...
        for (int b = -10; b < 10; ++b)
        {
            ...
            if ((center - Vector3{4, .2, 0}).length() > 0.9)
            {
                if (choose_mat < 0.8)
                {
                    ...
                    world.add(std::make_shared<MovingSphere>(center, center + Vector3{0, random_double(0, .5), 0}, 0.0, 1.0, 0.2,
...
HittableList earth()
{
    std::shared_ptr<ImageTexture> earth_texture = std::make_shared<ImageTexture>("Images/earthmap.jpg");
...
HittableList cornell_box()
{
    ...
    std::shared_ptr<Lambertian>   red = std::make_shared<Lambertian>(std::make_shared<SolidColor>(.65, .05, .05));
    std::shared_ptr<Lambertian>   white = std::make_shared<Lambertian>(std::make_shared<SolidColor>(.73, .73, .73));
    std::shared_ptr<Lambertian>   green = std::make_shared<Lambertian>(std::make_shared<SolidColor>(.12, .45, .15));
    std::shared_ptr<DiffuseLight> light = std::make_shared<DiffuseLight>(std::make_shared<SolidColor>(15, 15, 15));
...
HittableList final_scene()
{
    ...
    std::shared_ptr<Lambertian> emat = std::make_shared<Lambertian>(std::make_shared<ImageTexture>("Images/earthmap.jpg"));
    ...
    objects.add(std::make_shared<Translate>(
        std::make_shared<RotateY>(std::make_shared<BVHNode>(boxes2, 0.0, 1.0), 15), Vector3(-100, 270, 395)));
