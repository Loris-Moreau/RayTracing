...
// TODO: This is stupid ! Because the scene never changes at runtime, all of those buffers are actually of fixed size.
...
bool Refract(in Vector3 incidentVector, in Vector3 normal,
    ...
    // NOTE: we assume incident ray comes from the air which has a refraction index equal to 1.
    ...
    // If discriminant is lower than 0 we cannot refract the other medium.
...
bool IntersectWorld(in Ray ray, inout WorldIntersectionResult intersectionResult) {
    ...
    for (int rectangleIndex = 0; rectangleIndex < gRectangles.length(); ++rectangleIndex) {
        ...
        // rectangle's transform matrix is already inverted when creating the scene
        ...
        if (hit && t < intersectionResult.t && t > minHitDistance) {
            ...
            // If it's coming to the back side of the rectangle
...
// We do loop-based tracing instead of recursion-based tracing.
// You can write clean code by using recursion but I find recursion hard to understand.
// This is more straightforward and understandable.
Vector3 RaytraceWorld(in Ray ray, inout uint32_t randomState, inout uint32_t bounceCount) {
    ...
    for (uint32_t bounceIndex = 0; bounceIndex < 8; ++bounceIndex) {
        ...
        // We have to specify default values.
        ...
        if (isIntersect) {
            ...
            // We might calculate reflected and refracted ray separately and apply linear interpolation
...
void main() {
    ...
    // TODO: We can get the camera as an SSBO, so we don't have to calculate camera axis for every pixel.
    ...
    // That way we don't have to use version 4.6 and performance is the same as Atomic Counters.
