...
inline bool Refract(Vector3 incidentVector, Vector3 normal, float refractiveIndex, Vector3* refractionDirection)
{
    ...
    float discriminant = 1 - refractiveIndexRatio * refractiveIndexRatio * (1 - cosIncidentAngle * cosIncidentAngle);
    ...
    {
        *refractionDirection = incidentVector * refractiveIndexRatio + hitNormal * (refractiveIndexRatio * cosIncidentAngle - sqrtf(discriminant));
        
        return true;
    }
}

inline float Schlick(Vector3 incidentVector, Vector3 normal, float refractiveIndex)
{
    float cosIncidentAngle = Clamp(-1.0f, DotProduct(incidentVector, normal), 1.0f);
    float cosine;
    if (cosIncidentAngle > 0)
